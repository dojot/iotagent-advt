...
class Agent {
  ...
  enableModbusHandler(user: string, deviceId: string) {
    // It's magic.
    let enableObj = {
      "susiCommData": {
        "catalogID": 4,
        "autoUploadIntervalSec": 5,
        "requestItems": {
          "Modbus_Handler": {
            "e": [
              { "n": "Modbus_Handler" }
    ...
    console.log("Enabling modbus data...");
    let topic = "/cagent/" + user + "/" + deviceId + "/agentcallbackreq";
    console.log("Using topic " + topic);
    this.mqttContext.publish(topic, JSON.stringify(enableObj));
    console.log("... modbus is now enabled for device " + deviceId);
  }
  ...
  processMessage(topic: string, message: string): void {
    ...
    switch (tokens[tokens.length - 1]) {
      case "notify":
        ...
        // 'topic' should be something like /cagent/admin/0000000BABC80F4A/notify
        // Enabling modbus handler

        this.enableModbusHandler(tokens[2], tokens[3]);
        return;
    }
